...
const configurationSchema = Joi.object({
    enabled: Joi.boolean(),
    invocationCommand: Joi.string().lowercase().pattern(/^[a-z0-9][a-z0-9_-]*$/).max(50),
    provider: Joi.string().trim().min(1).max(80).allow('', null),
    model: Joi.string().trim().min(1).max(160).allow('', null),
    pointCost: Joi.number().integer().positive(),
    cooldownSeconds: Joi.number().integer().min(0).max(86400),
    cooldownScope: Joi.string().valid('global', 'streamer', 'session', 'participant'),
    maxInputChars: Joi.number().integer().min(1).max(12000),
    maxOutputChars: Joi.number().integer().min(1).max(12000),
    maxOutputTokenCount: Joi.number().integer().min(1).max(4096),
    timeoutMs: Joi.number().integer().min(100).max(30000)
}).min(1).required();
...
module.exports = class AiFeatureService extends Schmervice.Service {
    ...
    async updateConfiguration(userId, streamerId, input) {
        await this.server.services().authorizationService.requireCapability(userId, streamerId, 'editAiInstructions');
        const values = await configurationSchema.validateAsync(input);
        const { AiFeatureConfiguration } = this.server.models();
        const current = await AiFeatureConfiguration.query().findById(streamerId);
        const provider = values.provider === '' ? null : (values.provider ?? current?.provider ?? null);
        const model = values.model === '' ? null : (values.model ?? current?.model ?? null);
        const fields = {
            enabled: values.enabled ?? current?.enabled ?? false,
            invocationCommand: values.invocationCommand ?? current?.invocationCommand ?? 'ai',
            provider,
            model,
            configurationVersion: Crypto.randomUUID(),
            pricingPolicy: {
                type: 'fixed',
                pointCost: values.pointCost ?? Number(current?.pricingPolicy?.pointCost || 1)
            },
            cooldownSeconds: values.cooldownSeconds ?? current?.cooldownSeconds ?? 0,
            cooldownScope: values.cooldownScope ?? current?.cooldownScope ?? 'participant',
            maxInputChars: values.maxInputChars ?? current?.maxInputChars ?? 2000,
            maxOutputChars: values.maxOutputChars ?? current?.maxOutputChars ?? 2000,
            maxOutputTokenCount: values.maxOutputTokenCount ?? current?.maxOutputTokenCount ?? 512,
            timeoutMs: values.timeoutMs ?? current?.timeoutMs ?? 5000,
            updatedAt: new Date().toISOString()
        };

        if (!current) return AiFeatureConfiguration.query().insert({ streamerId, ...fields });
        return AiFeatureConfiguration.query().patchAndFetchById(streamerId, fields);
    }
