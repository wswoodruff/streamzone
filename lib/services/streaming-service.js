...
module.exports = class StreamingService extends Schmervice.Service {
    ...
    async createStream(userId, capability, stream) {
        ...
        return Stream.transaction(async (transaction) => {
            ...
            await this.reconcileStreamSession(created.streamSessionId, transaction);
            return created;
        });
    }
    ...
    async updateStream(userId, streamId, capability, changes) {
        ...
        return Stream.transaction(async (transaction) => {
            ...
            if (changes.sourceId && changes.sourceId !== stream.sourceId) {
                ...
                if (!target || target.streamerId !== stream.source.streamerId) throw new ResourceNotFoundError('Stream');
            }
            const updated = await Stream.query(transaction).patchAndFetchById(streamId, changes);
            await this.reconcileStreamSession(stream.streamSessionId, transaction);
            return updated;
        });
    }
    ...
    async updateStreamSession(userId, streamerId, sessionId, capability, changes) {
        ...
        return StreamSession.transaction(async (transaction) => {
            ...
            const updated = await StreamSession.query(transaction).patchAndFetchById(sessionId, { ...changes, updatedAt: new Date().toISOString() });
            if (session.status !== 'ended' && updated.status === 'ended') {
                const stateService = this.server.services().streamSessionStateService;
                if (stateService) await stateService.cleanupSession(sessionId, transaction);
                const runtimeState = this.server.app?.runtimeState;
                if (runtimeState) await runtimeState.clearSession(sessionId);
            }
            return updated;
        });
    }
    ...
    async reconcileStreamSession(sessionId, transaction) {
        ...
        const updated = await StreamSession.query(transaction).patchAndFetchById(sessionId, changes);
        if (session.status !== 'ended' && updated.status === 'ended') {
            const stateService = this.server.services().streamSessionStateService;
            if (stateService) await stateService.cleanupSession(sessionId, transaction);
            const runtimeState = this.server.app?.runtimeState;
