...
module.exports = class StreamManagementService extends Schmervice.Service {
    async overview(userId, requestedStreamerId) {
        ...
        const connections = await this.server.models().ProviderConnection.query().where({ streamerId: current.id, provider: 'youtube' }).whereNot('status', 'revoked');
        const permissions = {
        ...
        return {
            ...
            youtubeConnections: connections.map((connection) => ({
                id: connection.id,
                sourceId: connection.sourceId,
                channelName: sources.find((source) => source.id === connection.sourceId)?.displayName || sources.find((source) => source.id === connection.sourceId)?.channelId || connection.providerChannelId,
                channelId: connection.providerChannelId,
                active: connection.status === 'active',
                reconnect: connection.status !== 'active'
            })),
            hasYouTubeConnection: connections.length > 0,
