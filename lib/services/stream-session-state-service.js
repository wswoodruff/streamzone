    ...
    async compareAndSwap(streamSessionId, namespace, key, expectedVersion, value, options = {}) {
        ...
        return State.transaction(async (transaction) => {
            ...
            const totals = await State.query(transaction)
                ...
                .count({ count: 'id' })
                .first();
            const existingIsActive = existing && (!existing.expiresAt || existing.expiresAt > now.toISOString());
            const count = Number(totals.count || 0) - (existingIsActive ? 1 : 0) + 1;
            const bytes = Number(totals.bytes || 0) - (existingIsActive ? existing.sizeBytes : 0) + serialized.sizeBytes;
