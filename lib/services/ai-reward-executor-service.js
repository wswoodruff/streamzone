...
module.exports = class AiRewardExecutorService extends Schmervice.Service {
    ...
    async execute({ streamerId, redemptionId, input }) {
        if (typeof input !== 'string') throw new AiExecutionError('AI_INPUT_INVALID', 'AI reward input must be text.');
        const { RewardRedemption, RewardExecutorConfiguration, AiRewardExecution } = this.server.models();
        ...
        try {
            ...
            // User-controlled content is always a distinct user message. It can
            // never alter the trusted instruction or provider/model parameters.
            const request = {
            ...
            if (typeof output !== 'string' || output.length > config.maxOutputChars || Number(response?.usage?.outputTokens || 0) > config.maxTokens) {
            ...
            if (usage.costMicros > config.maxCostMicros) return this.fail(redemption, reservation.audit.id, 'moderated', started, usage, 'COST_LIMIT');
    ...
    async reserveBudget(redemption, config) {
        ...
        return AiRewardExecution.transaction(async (trx) => {
            ...
            const charged = (row) => row.status === 'dispatching' ? Number(row.reservedCostMicros) : Number(row.costMicros || 0);
            ...
            const [audit] = await trx('AiRewardExecution').insert({
                ...
                status: 'dispatching', reservedCostMicros: config.maxCostMicros
            }).returning('*');
    ...
    usage(usage = {}, config) {
        const values = { inputTokens: Number(usage.inputTokens || 0), outputTokens: Number(usage.outputTokens || 0), costMicros: Number(usage.costMicros || 0) };
        if (!Object.values(values).every(Number.isSafeInteger) || Object.values(values).some((v) => v < 0) || values.outputTokens > config.maxTokens) throw new AiExecutionError('AI_USAGE_INVALID', 'Provider returned invalid usage.');
        return values;
    }
    ...
    async completeAudit(id, status, started, usage = {}, errorCode = null, errorCategory = null) {
        const safe = usage ? { inputTokens: Number(usage.inputTokens || 0), outputTokens: Number(usage.outputTokens || 0), costMicros: Number(usage.costMicros || 0) } : {};
    ...
    publicAudit(row) { return { id: row.id, provider: row.provider, model: row.model, status: row.status, inputTokens: row.inputTokens, outputTokens: row.outputTokens, costMicros: row.costMicros, latencyMs: row.latencyMs, errorCode: row.errorCode, errorCategory: row.errorCategory }; }
