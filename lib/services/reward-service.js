...
const configSchemas = {
    deterministicBot: Joi.object({
        action: Joi.string().valid('sendChat', 'runCommand', 'applyRole').required(),
        parameters: Joi.object().default({})
    }).required(),
    manual: Joi.object().max(0).default({})
};
const transitions = Object.freeze({
    requested: ['reserved', 'rejected', 'cancelled'],
    reserved: ['fulfilled', 'rejected', 'cancelled'],
    rejected: ['refunded'],
    cancelled: ['refunded'],
    fulfilled: [],
    refunded: []
...
module.exports = class RewardService extends Schmervice.Service {
    async createReward(userId, streamerId, input) {
        ...
        return RewardDefinition.transaction(async (trx) => {
            ...
            if (reward.fulfillmentType === 'deterministicBot') {
                await trx('RewardExecutorConfiguration').insert({
    ...
    async updateReward(userId, streamerId, rewardId, input) {
        ...
        return RewardDefinition.transaction(async (trx) => {
            ...
            const typeChanged = type !== current.fulfillmentType;
            const configurationProvided = executorConfiguration !== undefined;
            let configuration;

            if (configurationProvided) configuration = await this._configuration(type, executorConfiguration);
            if (typeChanged && type === 'deterministicBot' && !configurationProvided) {
                throw new RewardError('EXECUTOR_CONFIGURATION_REQUIRED', 'Deterministic rewards require executor configuration.', 400);
            }

            if (Object.keys(fields).length) {
                await trx('RewardDefinition').where({ id: rewardId }).update({ ...fields, updatedAt: new Date().toISOString() });
            }

            if (type === 'manual') {
                await trx('RewardExecutorConfiguration').where({ rewardDefinitionId: rewardId }).delete();
            }
                ...
                }).onConflict('rewardDefinitionId').merge();
            }
    ...
    async redeem(input) {
        const { RewardDefinition } = this.server.models();
        return RewardDefinition.transaction(async (trx) => {
            const duplicate = await trx('RewardRedemption').where({
            ...
            const reward = await trx('RewardDefinition').where({
                id: input.rewardDefinitionId,
                streamerId: input.streamerId,
                enabled: true
            }).first();
            ...
            const [redemption] = await trx('RewardRedemption').insert({
                streamerId: input.streamerId,
                rewardDefinitionId: reward.id,
                chatUserId: input.chatUserId,
            ...
            await trx('PointLedgerEntry').insert({
                accountId: account.id,
                streamSessionId: input.streamSessionId || null,
                relatedExecutionId: String(redemption.id),
                delta: -pointCost,
                type: 'reserve',
                reason: `reward:${reward.name}`,
                actorType: 'user',
                actorId: String(input.chatUserId),
                idempotencyKey: `reward-reserve:${input.streamerId}:${input.chatUserId}:${input.idempotencyKey}`,
                createdAt: now
            });
            await trx('PointAccount').where({ id: account.id }).update({
                availableBalance: Number(account.availableBalance) - pointCost,
                updatedAt: now
            });
    ...
    async executeReserved(streamerId, redemptionId, actions) {
        ...
        if (reward.fulfillmentType !== 'deterministicBot') throw new RewardError('WRONG_EXECUTOR', 'Unsupported reward fulfillment type.', 400);

        const stored = await this.server.models().RewardExecutorConfiguration.query().findById(reward.id);
        if (!stored) throw new RewardError('EXECUTOR_CONFIGURATION_MISSING', 'Deterministic reward executor configuration is missing.');
        const result = await new DeterministicBotExecutor(actions).execute({ reward, redemption }, stored.configuration);
        if (!result.ok) {
            return {
                redemption: await this.fail(streamerId, redemptionId, result.failure.code, result.failure.details),
                execution: result
            };
        }
    ...
    async _transition(streamerId, id, next, failureCode = null, failureDetails = null) {
        ...
        return RewardRedemption.transaction(async (trx) => {
            ...
            const now = new Date().toISOString();
            await trx('RewardRedemption').where({ id }).update({
                status: next,
                failureCode,
                failureDetails,
                updatedAt: now,
                fulfilledAt: next === 'fulfilled' ? now : null
            });
    ...
    async _refund(redemption) {
        ...
        await this.server.services().pointEconomyService.refund({
            ...
            reason: `reward-refund:${row.failureCode}`,
            relatedExecutionId: String(row.id),
            actor: { type: 'system', id: 'reward-service' },
            idempotencyKey: `reward-refund:${row.id}`
        });
    ...
    async _configuration(type, config) {
        ...
        if (!schema) throw new RewardError('UNSUPPORTED_FULFILLMENT_TYPE', 'Unsupported reward fulfillment type.', 400);
        return schema.validateAsync(config);
    }

    _eligible(policy, provider, member) {
        return (!policy.providers.length || policy.providers.includes(provider)) &&
            (policy.membership === 'any' || (policy.membership === 'member') === member);
    }

    async _isMember(streamerId, chatUserId, trx) {
        const query = (trx || this.server.models().RewardDefinition.knex())('ChannelRelationship as r')
            .join('ChatIdentity as i', 'i.id', 'r.chatIdentityId')
            .join('Source as s', 's.id', 'r.sourceId')
            .where({ 's.streamerId': streamerId, 'i.chatUserId': chatUserId })
            .whereIn('r.relationship', ['subscriber', 'paid_member', 'moderator', 'vip', 'broadcaster'])
            .where((builder) => builder.whereNull('r.expiresAt').orWhere('r.expiresAt', '>', new Date().toISOString()));
        return Boolean(await query.first());
    }

    async _assertEligible(trx, reward, input) {
        const policy = typeof reward.eligibilityPolicy === 'string' ? JSON.parse(reward.eligibilityPolicy) : reward.eligibilityPolicy;
        if (!this._eligible(policy, input.provider, await this._isMember(input.streamerId, input.chatUserId, trx))) {
            throw new RewardError('NOT_ELIGIBLE', 'Participant is not eligible for this reward.', 403);
        }
    }

    async _assertLimits(trx, reward, input) {
        const successful = (query) => query.whereIn('status', ['reserved', 'fulfilled']);
...
module.exports.configSchemas = configSchemas;
