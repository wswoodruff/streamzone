...
module.exports = class RewardService extends Schmervice.Service {
    ...
    async executeReserved(streamerId, redemptionId, actions) {
        ...
        if (reward.fulfillmentType === 'ai') throw new RewardError('EXECUTOR_DISABLED', 'AI reward execution is not enabled.', 400);
    ...
    async _refund(redemption) {
        const { RewardRedemption } = this.server.models();
        return RewardRedemption.transaction(async (trx) => {
            const row = await trx('RewardRedemption').where({ id: redemption.id }).forUpdate().first();
            ...
            const account = await trx('PointAccount').where({ streamerId: row.streamerId, chatUserId: row.chatUserId }).forUpdate().first();
            const key = `reward-refund:${row.id}`;
            const existing = await trx('PointLedgerEntry').where({ idempotencyKey: key }).first();
            if (!existing) {
                await trx('PointLedgerEntry').insert({ accountId: account.id, streamSessionId: row.streamSessionId, relatedExecutionId: String(row.id), delta: row.pointCost, type: 'refund', reason: `reward-refund:${row.failureCode}`, actorType: 'system', actorId: 'reward-service', idempotencyKey: key });
                await trx('PointAccount').where({ id: account.id }).update({ availableBalance: Number(account.availableBalance) + Number(row.pointCost), updatedAt: new Date().toISOString() });
            }
            await trx('RewardRedemption').where({ id: row.id }).update({ status: 'refunded', updatedAt: new Date().toISOString() });
            return { ...row, status: 'refunded' };
        });
    }

    async _configuration(type, config) {
        if (type === 'ai') throw new RewardError('EXECUTOR_DISABLED', 'AI reward execution is not enabled.', 400);
