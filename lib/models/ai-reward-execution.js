...
module.exports = class AiRewardExecution extends Schwifty.Model {
    ...
    static get joiSchema() {
        return Joi.object({
            ...
            inputTokens: Joi.number().integer().min(0).allow(null), outputTokens: Joi.number().integer().min(0).allow(null),
            costMicros: Joi.number().integer().min(0).allow(null), reservedCostMicros: Joi.number().integer().min(0).required(),
