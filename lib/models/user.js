...
module.exports = class User extends Schwifty.Model {
    ...
    static get relationMappings() {
        ...
        const ExternalLoginIdentity = require('./external-login-identity');

        return {
            claimedChatUsers: {
                relation: Schwifty.Model.HasManyRelation,
                modelClass: ChatUser,
                join: { from: 'User.id', to: 'ChatUser.userId' }
            },
            memberships: {
                relation: Schwifty.Model.HasManyRelation,
                modelClass: StreamerMembership,
                join: { from: 'User.id', to: 'StreamerMembership.userId' }
            },
            externalLoginIdentities: { relation: Schwifty.Model.HasManyRelation, modelClass: ExternalLoginIdentity, join: { from: 'User.id', to: 'ExternalLoginIdentity.userId' } }
        };
    }
