...
class PromptComposer {
    composeRequest({ platformPolicyVersion, streamerInstructionVersion, runtimeContext, conversationSummary, participantInput, provider, model, maxTokens, signal }) {
        ...
        if (typeof provider !== 'string' || typeof model !== 'string' || !Number.isSafeInteger(maxTokens) || maxTokens < 1) throw new PromptCompositionError('AI_REQUEST_INVALID', 'Trusted provider parameters are invalid.');

        return Object.freeze({
            provider, model, maxTokens, signal,
