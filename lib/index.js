...
const { loadCookieEnvironment } = require('./config/environment');
    ...
    register: async (server, options) => {
        ...
        server.auth.strategy('session', 'cookie', {
            cookie: {
                ...
                password: process.env.NODE_ENV === 'test' && !process.env.COOKIE_PASSWORD ? 'test-only-cookie-password-not-for-deployments' : loadCookieEnvironment().password,
